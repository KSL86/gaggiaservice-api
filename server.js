...
dns.setDefaultResultOrder("ipv4first"); // Force IPv4 — Render free tier doesn't support IPv6
...
const app = express();
app.use(cors({
  origin: ["https://gaggiaservice.no", "https://www.gaggiaservice.no", "http://localhost:3000"],
  credentials: true,
}));
...
let twilio, resend;
if (TWILIO_SID && TWILIO_TOKEN) { twilio = require("twilio")(TWILIO_SID, TWILIO_TOKEN); console.log("✅ Twilio ready"); }
if (RESEND_KEY) { const { Resend } = require("resend"); resend = new Resend(RESEND_KEY); console.log("✅ Resend ready"); }

// ─── Helpers ───
const genServiceNr = () => "SRV-" + Date.now().toString(36).slice(-4).toUpperCase() + crypto.randomBytes(2).toString("hex").toUpperCase();
...
function auth(req, res, next) {
  ...
  if (!header || !header.startsWith("Bearer ")) return sendError(res, 401, "Ikke autorisert");
  try {
    req.admin = jwt.verify(header.slice(7), JWT_SECRET);
    next();
  } catch { return sendError(res, 401, "Ugyldig eller utløpt token"); }
}
...
app.post("/api/login", async (req, res) => {
  ...
  if (!username || !password) return sendError(res, 400, "Mangler brukernavn eller passord");

  const { rows } = await pool.query("SELECT * FROM admin_users WHERE username = $1", [username]);
  if (rows.length === 0) return sendError(res, 401, "Feil brukernavn eller passord");

  const valid = await bcrypt.compare(password, rows[0].password_hash);
  if (!valid) return sendError(res, 401, "Feil brukernavn eller passord");

  const token = jwt.sign({ id: rows[0].id, username: rows[0].username, name: rows[0].name }, JWT_SECRET, { expiresIn: JWT_EXPIRES });
  res.json({ token, name: rows[0].name, expiresIn: JWT_EXPIRES });
});

app.post("/api/change-password", auth, async (req, res) => {
  const { currentPassword, newPassword } = req.body;
  if (!currentPassword || !newPassword || newPassword.length < 8) return sendError(res, 400, "Nytt passord må være minst 8 tegn");

  const { rows } = await pool.query("SELECT password_hash FROM admin_users WHERE id = $1", [req.admin.id]);
  const valid = await bcrypt.compare(currentPassword, rows[0].password_hash);
  if (!valid) return sendError(res, 401, "Feil nåværende passord");
...
app.post("/api/customers", auth, async (req, res) => {
  ...
  if (!name || (!phone && !email)) return sendError(res, 400, "Navn + telefon eller e-post kreves");
  const { rows } = await pool.query(
    "INSERT INTO customers (name, phone, email, address, zip, city) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
    [name.trim(), phone?.trim() || null, email?.trim() || null, address?.trim() || null, zip?.trim() || null, city?.trim() || null]
  );
  res.json(rows[0]);
});
...
app.put("/api/customers/:id", auth, async (req, res) => {
  ...
  const { rows } = await pool.query(
    "UPDATE customers SET name=$1, phone=$2, email=$3, updated_at=NOW() WHERE id=$4 AND deleted_at IS NULL RETURNING *",
...
app.delete("/api/customers/:id", auth, async (req, res) => {
  ...
  await pool.query("UPDATE customers SET deleted_at=NOW(), name='[slettet]', phone=NULL, email=NULL, address=NULL, zip=NULL, city=NULL WHERE id=$1", [req.params.id]);
  await pool.query("INSERT INTO gdpr_log (action, subject_id, admin_id, details) VALUES ($1,$2,$3,$4)",
...
app.get("/api/machines", auth, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT m.*, c.name as customer_name FROM machines m
     JOIN customers c ON m.customer_id = c.id WHERE c.deleted_at IS NULL ORDER BY m.created_at DESC`
  );
  res.json(rows);
});
...
app.post("/api/machines", auth, async (req, res) => {
  ...
  if (!customer_id || !model) return sendError(res, 400, "Kunde og modell kreves");
  const { rows } = await pool.query(
    "INSERT INTO machines (customer_id, brand, model, model_code, serial) VALUES ($1,$2,$3,$4,$5) RETURNING *",
...
app.get("/api/orders", auth, async (req, res) => {
  ...
  // Attach status history, used parts
  for (const o of rows) {
    const sh = await pool.query("SELECT * FROM status_history WHERE order_id=$1 ORDER BY created_at", [o.id]);
    o.status_history = sh.rows;
    const up = await pool.query("SELECT * FROM used_parts WHERE order_id=$1 ORDER BY created_at", [o.id]);
    o.used_parts = up.rows;
    const wl = await pool.query("SELECT * FROM work_logs WHERE order_id=$1 ORDER BY created_at DESC", [o.id]);
    o.work_logs = wl.rows;
  }
  res.json(rows);
});
...
app.post("/api/orders", auth, async (req, res) => {
  ...
  if (!customer_id || !machine_id) return sendError(res, 400, "Kunde og maskin kreves");
  ...
  try {
    ...
    const { rows } = await client.query(
      "INSERT INTO orders (service_nr, customer_id, machine_id, description, delivery_method) VALUES ($1,$2,$3,$4,$5) RETURNING *",
      [service_nr, customer_id, machine_id, description || null, delivery_method || null]
    );
    await client.query("INSERT INTO status_history (order_id, status, note) VALUES ($1, 'registered', 'Ordre opprettet')", [rows[0].id]);
    ...
    if (cust) await sendNotification(cust, rows[0], "registered", mach);
...
app.put("/api/orders/:id/status", auth, async (req, res) => {
  ...
  try {
    await client.query("BEGIN");
    const { rows } = await client.query("UPDATE orders SET status=$1, updated_at=NOW() WHERE id=$2 RETURNING *", [status, req.params.id]);
    if (!rows.length) { await client.query("ROLLBACK"); return sendError(res, 404, "Ordre ikke funnet"); }
    await client.query("INSERT INTO status_history (order_id, status, note) VALUES ($1,$2,$3)", [req.params.id, status, note || null]);
    await client.query("COMMIT");
    ...
    if (cust) await sendNotification(cust, o, status, mach);
...
app.put("/api/orders/:id/faults", auth, async (req, res) => {
  ...
  const { rows } = await pool.query("UPDATE orders SET fault_codes=$1, updated_at=NOW() WHERE id=$2 RETURNING *", [fault_codes || [], req.params.id]);
  res.json(rows[0]);
});
...
app.post("/api/orders/:id/parts", auth, async (req, res) => {
  ...
  const { rows } = await pool.query(
    "INSERT INTO used_parts (order_id, part_nr, name, price, qty) VALUES ($1,$2,$3,$4,$5) RETURNING *",
...
app.delete("/api/orders/:id/parts/:partId", auth, async (req, res) => {
  await pool.query("DELETE FROM used_parts WHERE id=$1 AND order_id=$2", [req.params.partId, req.params.id]);
  res.json({ ok: true });
});
...
app.post("/api/orders/:id/logs", auth, async (req, res) => {
  ...
  const { rows } = await pool.query(
    "INSERT INTO work_logs (order_id, description, parts, minutes) VALUES ($1,$2,$3,$4) RETURNING *",
...
app.get("/api/stock", auth, async (req, res) => {
  ...
  const map = {};
  rows.forEach(r => { map[r.part_nr] = r.qty; });
  res.json(map);
});
...
app.put("/api/stock/:partNr", auth, async (req, res) => {
  const { qty } = req.body;
  await pool.query(
    `INSERT INTO stock (part_nr, qty, updated_at) VALUES ($1, $2, NOW())
...
app.get("/api/notifications", auth, async (req, res) => {
  const { rows } = await pool.query("SELECT * FROM notifications ORDER BY sent_at DESC LIMIT 200");
  res.json(rows);
});
...
app.get("/api/portal/:serviceNr", async (req, res) => {
  const { rows } = await pool.query(
    `SELECT o.id, o.service_nr, o.status, o.description, o.delivery_method, o.fault_codes, o.created_at,
     ...
     FROM orders o JOIN machines m ON o.machine_id = m.id
     WHERE LOWER(o.service_nr) = LOWER($1)`, [req.params.serviceNr]
  );
  if (!rows.length) return sendError(res, 404, "Ordre ikke funnet");

  const o = rows[0];
  const sh = await pool.query("SELECT status, note, created_at FROM status_history WHERE order_id=$1 ORDER BY created_at", [o.id]);
  o.status_history = sh.rows;

  // Don't expose internal customer data to portal
  delete o.id;
  res.json(o);
});

app.post("/api/portal/register", async (req, res) => {
  const { name, phone, email, address, zip, city, model_code, serial, description, symptoms, delivery_method } = req.body;
  if (!name || (!phone && !email) || !model_code) return sendError(res, 400, "Mangler påkrevde felt");

  // Find model name from code
  const model_name = req.body.model_name || model_code;
  const service_nr = genServiceNr();

  // Build full description from symptoms + free text
  let fullDesc = "";
  if (symptoms && symptoms.length > 0) {
    fullDesc = "Symptomer:\n" + symptoms.map(s => "• " + s).join("\n");
  }
  if (description) fullDesc += (fullDesc ? "\n\nTilleggsinformasjon:\n" : "") + description;

  const addrStr = [address, zip, city].filter(Boolean).map(s => s.trim()).filter(Boolean).join(", ");
  if (addrStr) fullDesc += "\n\nKundeadresse: " + addrStr;

  const deliveryLabels = { post: "Sendes per post", dropoff: "Leveres Gjøvik", partner: "Leveres ServiceKompaniet Oslo" };
  ...
  try {
    ...
    const mach = (await client.query(
      "INSERT INTO machines (customer_id, brand, model, model_code, serial) VALUES ($1,'Gaggia',$2,$3,$4) RETURNING *",
...
// GDPR
// ═══════════════════════════════════════
// Customer data export (Article 15 — right of access)
app.get("/api/gdpr/export/:customerId", auth, async (req, res) => {
  const cid = req.params.customerId;
  const customer = (await pool.query("SELECT * FROM customers WHERE id=$1", [cid])).rows[0];
  if (!customer) return sendError(res, 404, "Kunde ikke funnet");

  const machines = (await pool.query("SELECT * FROM machines WHERE customer_id=$1", [cid])).rows;
  const orders = (await pool.query("SELECT * FROM orders WHERE customer_id=$1", [cid])).rows;
  const notifications = (await pool.query("SELECT * FROM notifications WHERE customer_id=$1", [cid])).rows;
...
// Customer data deletion (Article 17 — right to erasure)
app.delete("/api/gdpr/erase/:customerId", auth, async (req, res) => {
  const cid = req.params.customerId;

  // Soft delete — anonymize personal data but keep order history for business records
  await pool.query("UPDATE customers SET name='[slettet]', phone=NULL, email=NULL, address=NULL, zip=NULL, city=NULL, deleted_at=NOW() WHERE id=$1", [cid]);
  await pool.query("INSERT INTO gdpr_log (action, subject_id, admin_id, details) VALUES ($1,$2,$3,$4)",
...
app.get("/api/gdpr/log", auth, async (req, res) => {
  const { rows } = await pool.query("SELECT * FROM gdpr_log ORDER BY created_at DESC LIMIT 100");
  res.json(rows);
});
...
async function sendNotification(cust, order, status, mach) {
  ...
  // Log to database
  await pool.query(
    "INSERT INTO notifications (order_id, customer_id, type, recipient, subject, message, status) VALUES ($1,$2,$3,$4,$5,$6,$7)",
    [order.id, cust.id, type, to, subject, msg, status]
  );
  ...
  try {
    if (type === "sms" && twilio) {
      let phone = to.replace(/\s/g, "");
      if (!phone.startsWith("+")) phone = "+47" + phone;
      await twilio.messages.create({ body: msg, from: TWILIO_FROM, to: phone });
      await pool.query("UPDATE notifications SET delivered=true WHERE order_id=$1 AND type='sms' ORDER BY sent_at DESC LIMIT 1", [order.id]);
      console.log(`📱 SMS → ${phone}`);
    } else if (type === "email" && resend) {
      await resend.emails.send({
        from: EMAIL_FROM, to: [to], subject,
        text: msg,
        html: `<div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:20px">
          <div style="border-bottom:2px solid #1D4ED8;padding-bottom:10px;margin-bottom:16px">
            <strong style="font-size:16px">🔋 Battericentralen</strong>
      ...
      await pool.query("UPDATE notifications SET delivered=true WHERE order_id=$1 AND type='email' ORDER BY sent_at DESC LIMIT 1", [order.id]);
      console.log(`✉️ E-post → ${to}`);
    }
  } catch (err) {
    console.error(`❌ ${type} feil:`, err.message);
    await pool.query("UPDATE notifications SET error=$1 WHERE order_id=$2 ORDER BY sent_at DESC LIMIT 1", [err.message, order.id]);
  }
}
